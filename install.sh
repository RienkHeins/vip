...
unzip_reference() {
  ...
  if [ "${assembly}" == "ALL" ] || [ "${assembly}" == "GRCh37" ]; then
    ...
    if [ ! -d "${download_dir}/GRCh37/human_g1k_v37_ref_cache" ]; then
      ...
      tar -zcvf "${download_dir}/GRCh37/human_g1k_v37.ref_cache.tar.gz" "${download_dir}/GRCh37/human_g1k_v37_ref_cache"
    else
  ...
  if [ "${assembly}" == "ALL" ] || [ "${assembly}" == "GRCh38" ]; then
    ...
    if [ ! -d "${download_dir}/GRCh38/GCA_000001405.15_GRCh38_no_alt_analysis_set_ref_cache" ]; then
      ...
      tar -zcvf "${download_dir}/GRCh38/GCA_000001405.15_GRCh38_no_alt_analysis_set.ref_cache.tar.gz" "${download_dir}/GRCh38/GCA_000001405.15_GRCh38_no_alt_analysis_set_ref_cache"
    else
      echo -e "skipping extraction of reference cache for GRCh37: already exists"
    fi
  fi
